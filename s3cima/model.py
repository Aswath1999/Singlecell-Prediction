...
def train_model(ntime_points,train_samples, train_phenotypes, outdir,
    ...
    print('Number of markers: %d' % nmark)
    ...
    # batch size - TODO this should not be hardcoded
    bs = 200
    n_classes = 1
    ...
    for irun in range(nrun):
        ...
        # Filename keras to keep up with new version
        filepath = os.path.join(outdir, 'nnet_run_%d.keras' % irun)
        # run_dir = os.path.join(outdir, f"run_{irun}")
        # os.makedirs(run_dir, exist_ok=True)

        # filepath = os.path.join(run_dir, "model.keras")
        try:
            if not regression:
                if (ntime_points==1):
    ...
    best_3_nets = [w_store[int(i)] for i in model_sorted_idx]
...
def float32(k):
    return np.asarray(k, dtype='float32')

def int32(k):
    return np.asarray(k, dtype='int32')
...
def build_model(ntime_points, ncell, nmark, nfilter, coeff_l1, coeff_l2, k, dropout, dropout_p, regression, n_classes, lr=0.01,
    ...
    # output_shape=(nfilter,),
    print(x_t_conv[str(nt)])
    print(x_t_conv[str(nt)].shape)
    
    pool[str(nt)] = layers.Lambda(select_top, 
                                  output_shape=lambda s: (s[0], s[2]), 
                                  arguments={'k': k, 'type' : selection_type})(x_t_conv[str(nt)])
