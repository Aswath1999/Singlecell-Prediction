...
"""
Evaluation script for MultiHeadMarkerAdapterResNet18
---------------------------------------------------
• Loads BEST multi-head adapter model
• Evaluates on TEST patient (last patient)
• Uses y_major labels (PRIMARY head)
• Prints:
    - Accuracy
    - Recall per class
    - Confusion matrix
...
BEST_PATH = "best_multihead_markerbatchnorm_adapter.pt"
...
# MODEL
# ============================================================
class MultiHeadMarkerAdapterResNet18(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        x = self.adapter(x)
        f = torch.flatten(self.feat(x), 1)
        return self.head_major(f), self.head_center(f)
...
def main():
    ...
    # Load model
    model = MultiHeadMarkerAdapterResNet18().to(DEVICE)
